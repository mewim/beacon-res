...
router.get('/', function (req, res) {
    DB.conn(function (db) {
        ...
        beacons.find({}).toArray(function (error, result) {
            if (error) {
                res.status(500).send({
                    success: false,
                    message: error
        ...
        if (!error && response.statusCode == 200) {
          ...
          DB.conn(function(db) {
            ...
            beacons_version.remove({}, function(error, number_removed) {
              if (error) {
                ...
                beacons_version.insert(parsed_version, function(
                  error,
                  result
                ) {
                  if (error) {
                    ...
                    request(
                      "http://minrva-dev.library.illinois.edu:8080/estimote/rest/v1.0/beacons",
                      function(error, response, body) {
                        var beacons = db.collection("beacons");
                        beacons_version.remove({}, function(
                          error,
                          number_removed
                        ) {
                          if (!error && response.statusCode == 200) {
                            ...
                            res.status(500).send({
                              success: false,
                              message:
                                "Cannot download beacons from remote server."
                            });
                          }
                        });
                      }
          ...
          res
            ...
            .send({
              success: false,
              message:
                "Cannot download beacon version from remote server."
            });
        }
    });
});
